...
import { MapPin, Info, Search, Maximize2, Minimize2 } from "lucide-react";
...
const RegionalChart: React.FC<RegionalChartProps> = ({ data, isFullPage = false }) => {
  ...
  const chartData = data.regions
    ...
    .map((region) => ({
      ...
      demand: Math.floor(Math.random() * 100) + 30, // Random value between 30-130
    }));
  ...
  const handleFullscreenToggle = () => {
    if (isFullPage) {
      ...
      router.push('/forecasting/regional');
    }
  };
        ...
        <CardContent className="p-4">
              ...
              >
                ...
                >
                  {chartData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={selectedRegion === entry.name ? '#16a34a' : '#4ade80'}
                ...
                >
                  {chartData.map((entry, index) => (
                    <Cell
                      ...
                      fill={selectedRegion === entry.name ? '#d97706' : '#f59e0b'}
                      className="cursor-pointer transition-colors duration-200"
                    />
                  ))}
                </Bar>
          ...
          {selectedRegion && (
            <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              <p className="text-green-700">{selectedRegion}</p>
              <div className="mt-2 grid grid-cols-2 gap-4">
                <div className="bg-white p-3 rounded-lg border border-green-100">
                  <p className="text-sm text-gray-600">Production Index</p>
                  <p className="text-lg font-semibold text-green-600">
                    {chartData.find(d => d.name === selectedRegion)?.value}
                  </p>
                </div>
                <div className="bg-white p-3 rounded-lg border border-green-100">
                  <p className="text-sm text-gray-600">Demand Index</p>
                  <p className="text-lg font-semibold text-amber-600">
                    {chartData.find(d => d.name === selectedRegion)?.demand}
                  </p>
                </div>
              </div>
            </div>
