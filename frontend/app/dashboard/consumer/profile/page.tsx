  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      {user?.consumerProfile && (
        <Card>
          <CardHeader><CardTitle>Consumer Details</CardTitle></CardHeader>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div><span className="text-gray-500">Type:</span> <span className="font-medium">{user.consumerProfile.type}</span></div>
            <div><span className="text-gray-500">Business Name:</span> <span className="font-medium">{user.consumerProfile.businessName || 'N/A'}</span></div>
          </div>
        </Card>
      )}
    </div>
