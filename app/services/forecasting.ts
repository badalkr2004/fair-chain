...
class ForecastingService {
  ...
  async getAllCrops(): Promise<string[]> {
    try {
      ...
      return [
        'Arhar/Tur', 'Bajra', 'Barley', 'Castor seed', 'Coriander',
        'Dry chillies', 'Garlic', 'Ginger', 'Gram', 'Groundnut',
        'Horse-gram', 'Jowar', 'Jute', 'Khesari', 'Linseed',
        'Maize', 'Masoor', 'Moong(Green Gram)', 'Onion', 'Potato',
        'Ragi', 'Rapeseed &Mustard', 'Rice', 'Sugarcane', 'Sunflower',
        'Turmeric', 'Urad', 'Wheat'
      ];
    }
  }
  ...
  async getCropForecast(params: CropForecastParams): Promise<CropForecastResponse> {
    try {
      const response = await axios.post(`${API_BASE_URL}/forecast/crop`, params);
      return response.data;
    } catch (error) {
  ...
  async getOptimalCrops(params: OptimalCropsParams): Promise<OptimalCropResponse> {
    try {
      ...
      const yieldTrends: Array<'increasing' | 'stable' | 'decreasing'> = ['increasing', 'stable', 'decreasing'];
      const profitPotentials: Array<'high' | 'medium' | 'low'> = ['high', 'medium', 'low'];
      
      return {
        status: 'success',
        region: params.region,
        optimal_crops: crops.slice(0, params.top_n).map(crop => {
          const yieldTrend = yieldTrends[Math.floor(Math.random() * yieldTrends.length)];
          const growthPotential = yieldTrend === 'increasing' ? 
            Math.random() * 30 : 
            yieldTrend === 'stable' ? 
              Math.random() * 5 : 
              -Math.random() * 20;
          ...
          return {
            ...
            profit_potential: profitPotentials[Math.floor(Math.random() * profitPotentials.length)]
          };
        }),
  ...
  // Get crop calendar
  async getCropCalendar(params: CropCalendarParams): Promise<CropCalendarResponse> {
    try {
      const response = await axios.post(`${API_BASE_URL}/crop-calendar`, params);
      return response.data;
    } catch (error) {
      ...
      cropCalendars[params.crop_name] = [
        {
          season: randomSeason,
          planting_time: randomSeason === 'Spring' ? 'March-April' : 
                         randomSeason === 'Summer' ? 'June-July' : 
                         randomSeason === 'Autumn' ? 'August-September' : 'November-December',
          harvesting_time: randomSeason === 'Spring' ? 'June-July' : 
                           randomSeason === 'Summer' ? 'September-October' : 
                           randomSeason === 'Autumn' ? 'November-December' : 'February-March'
        }
      ];
  ...
  async getMarketPrices(cropName: string): Promise<MarketPriceResponse> {
    try {
      ...
      const priceTrend = ['increasing', 'decreasing', 'stable'][Math.floor(Math.random() * 3)];
  ...
  async getRegionalDemand(params: RegionalDemandParams): Promise<RegionalDemandResponse> {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/forecast/regional_demand`, params);
      return response.data;
    } catch (error) {
      console.error('Error fetching regional demand:', error);
      // Return mock data if API fails based on the new format
      const mockCrops = ['Wheat', 'Potato', 'Sugarcane', 'Masoor', 'Barley'].slice(0, params.top_n);
      ...
      const mockResponse: any = mockCrops.map((crop, index) => {
        ...
        return {
          ...
          forecast_years: Array.from({ length: params.periods }, (_, i) => 2023 + i)
        };
      });
